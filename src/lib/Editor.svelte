<script lang="ts">
	...
	import { createEditor, $createTextNode as createTextNode, $getSelection as getSelection, $isRangeSelection as isRangeSelection, KEY_DOWN_COMMAND, COMMAND_PRIORITY_LOW, COMMAND_PRIORITY_NORMAL, type EditorState } from 'lexical';
	...
	import { registerMarkdownShortcuts, TRANSFORMERS, type TextMatchTransformer } from '@lexical/markdown';
	import { ListNode, ListItemNode } from '@lexical/list';
	import { LinkNode } from '@lexical/link';
	import { CodeNode, CodeHighlightNode } from '@lexical/code';
	import { TargetNode, $createTargetNode as createTargetNode, $isTargetNode as isTargetNode, $toggleTarget as toggleTarget, TOGGLE_TARGET_COMMAND } from '$lib/nodes/TargetNode';
	import type { EditorTheme } from '$lib/themes/types';
	import classes from '$lib/themes/classes';
	import { writable } from 'svelte/store';

	const STORAGE_KEY = 'backseatwriter:editor-state';
	...
	$: if (styleEl) {
		styleEl.textContent = theme.styles;
	}
	...
	onMount(() => {
		...
		if (saved) {
			...
			editorStateJson.set(JSON.stringify(JSON.parse(saved), null, 2));
		}
					...
					if (event.key === 'h' && event.shiftKey && (event.metaKey || event.ctrlKey)) {
						...
						if (isRangeSelection(selection)) {
							...
							if (targetParent) {
								...
								editor.dispatchCommand(TOGGLE_TARGET_COMMAND, crypto.randomUUID());
							}
						}
						return true;
					}
